...
class Game(object):
    ...
    def __init__(self, session_id, args,use_cache=True,use_pretrained=False):
        # print(session_id)
        ...
        if not use_cache:
            ...
            self.dialogue_embeddings =self.dialogue
            # print(self.output_events)
            print("begin_parse")
            self.parse_traj()
            self.parse_event()
            print("end_parse")
            with open(os.path.join(args.dataset_path,session_id,"event_bert_emb.pkl"),"wb") as f:
                pkl.dump(self.output_events,f)
            # exit()
        if use_pretrained:
            self.output_events=pkl.load(open(os.path.join(args.dataset_path,session_id,"event_bert_emb.pkl"),'rb'))
        else:
        ...
        self.parse_config()
        self.load_plan()
        for idx, event in enumerate(self.output_events):
            self.output_events[idx]['detailed_plan'] = self.detailed_plan[event['frame']]
        
        self.exception_events=[]
        self.non_exception_events=[]
    ...
    def find_exception(self):
        ...
        except_id=[]
        with open("test_exception.json","r") as f:
            exception_list = json.load(f)
            for exception in exception_list :
                if self.session_id[-14:] in exception:
                    count=3
                    for i in range(len(self.output_events)):
                        event=self.output_events[i]
                        if event['type'] == 'DialogueMove' and (event['act']['exchange']>=exception[1]) and (event['act']['exchange']<=exception[2]):
                            self.exception_events.append(event)
                            except_id.append(i)
                        if  event['type'].strip() == 'PhysicalAction' and (event['frame']>=exception[1]) and count>=0:
                            self.exception_events.append(event)
                            except_id.append(i)
                            count-=1
                    
                    print(len(self.exception_events) )   
        for i in range(len(self.output_events)):
            if i not in except_id:
                self.non_exception_events.append(self.output_events[i])
    ...
    def parse_event(self):
        ...
        for event in self.output_events:
            ...
            dialogue=torch.tensor(np.array(dialogue))
            ...
            # event["dialog_history_token"]=self.dialogue_token[dlg_idx] if dlg_idx else torch.zeros([0])
            # event["dialog_history_mask"]=self.dialogue_mask[dlg_idx] if dlg_idx else torch.zeros([0])
            # print(event["dialog_history_token"].shape)
            ...
            if event["gt_physical_type"]!=None:
                event["gt_physical_type"]=torch.tensor(event["gt_physical_type"])-1
    ...
    def parse_map(self):
        ...
        if not os.path.exists(pkl_file):
            ...
            model = BertModel.from_pretrained('bert-base-uncased', output_hidden_states=True).to(self.args.device, non_blocking=True)
            model.eval()
            ...
            for name in self.street_names:
                    ...
                    token_ids = encoded_dict['input_ids'].to(self.args.device, non_blocking=True)
    ...
    def load_plan(self):
        def decode_landmark(str,known_landmarks):
            landmark = [x for x in Landmarks if x.name.lower() in str.lower()]
            if not landmark:
                return None
            landmark_data = [x for x in known_landmarks if landmark[0].name.lower() in x.asset.lower()]
            if not landmark_data:
                return None
            return landmark_data[0]
        ...
        with open(os.path.join(self.dataset_path, self.session_id, 'detailed_plan.json')) as f:
            detailed_plan = json.load(f)
            self.detailed_plan = {
                x['frame'] : {
                    'junctions': [self.all_junction_id[y] for y in x['junctions']] if x['junctions'] else None, 
                    'objective' : None if x['objective'] is None else decode_landmark(x['objective'][0],self.known_landmarks)} 
                for x in detailed_plan}
    ...
    def load_camera(self):
        ...
        #     resnet50 = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_resnet50', pretrained=True).to(self.args.device, non_blocking=True)
        #     resnet50.eval()
        #     rgb = {}
        #     with torch.no_grad():
        ...
        #             v = load_image(file)
        #             rgb[k] = resnet50(torch.tensor(np.array([v])).permute(0,3,1,2)[:,0:3,:,:].float().to(self.args.device, non_blocking=True)).cpu().data.numpy()[0]
        ...
        #     model = DetrModel.from_pretrained("facebook/detr-resnet-50").to(self.args.device, non_blocking=True)
        #     model.eval()
        ...
        #         inputs = {k:v.to(self.args.device, non_blocking=True) for k,v in inputs.items()}
        #         with torch.no_grad():
    ...
    def load_dialogue(self):
        ...
        if self.use_cache==False:
            save_path = f'/gpfs/accounts/chaijy_root/chaijy0/owenhji/model_path/ablation13_seed0.torch'

            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
            model = TOTO(self.args).to(self.args.device, non_blocking=True)
            ...
            for frame in json.load(open(os.path.join(self.dataset_path, self.session_id, 'annotated_log.json'))):
                if frame['type'] == 'DialogueMove':
                    ...
                    segment_ids = torch.ones(token_ids.size()).long().to(self.args.device, non_blocking=True)
                    with torch.no_grad():
                        outputs = model.embedding_encoder.bert(input_ids=token_ids, token_type_ids=segment_ids)
                    outputs = outputs[1][0].cpu().data.numpy()
                    outputs = np.concatenate([[int(frame['from']=='dorothy')],[int(frame['to']=='dorothy')],outputs])
                    self.dialogue.append ((frame["frame"],outputs))#{"from": frame['from'], "to": frame['to'], "val": outputs}
        ...
        #     if frame['type'] == 'DialogueMove':
        #         utterance = frame['utterance_gt'].replace('\u2019',',')
        #         self.dialogue_list.append(utterance)
        # # print(self.dialogue_list)
        # encoded_dict = tokenizer.batch_encode_plus(
        #                 self.dialogue_list,  # Sentence to encode.
        #                 add_special_tokens=True,  # Add '[CLS]' and '[SEP]'
        #                 return_tensors='pt',  # Return pytorch tensors.
        #                 padding=True,
        #     )
        # self.dialogue_token=encoded_dict['input_ids']
        # self.dialogue_mask=encoded_dict['attention_mask']
        # print(self.dialogue_token.shape)
        # print([a for a,b in self.dialogue])
    ...
    def __prep_detailed_plan(self, idx):
        if idx in self.detailed_plan:
            print(self.detailed_plan[idx] )
            return self.detailed_plan[idx] 
        else:
            return {'frame': idx, 'x': 0.0, 'y': 0.0, 'yaw': 0.0, 'junction_id': -1, 'road_id': -1, 'lane_id': -1}
    ...
    def __getitem__(self, slc):
        ...
        retval = {
            ...
            'detailed_plan' : self.__prep_detailed_plan(idx),
        }
