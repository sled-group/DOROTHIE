...
def main(proc_idx, events, args):
    ...
    if proc_idx is None:
        ...
        if args.is_exception:
            ...
            if args.seen:
                out_file = open(f'seen_non_exception.csv','a')
            else:
                out_file = open(f'unseen_non_exception.csv','a')   
        else:
            if args.seen:
                out_file = open(f'seen.csv','a')
            else:
                out_file = open(f'unseen.csv','a')
        close_file = True
        # args.save_path = f'/gpfs/accounts/chaijy_root/chaijy0/owenhji/model_path/ablation{proc_idx}_seed{args.seed}.torch'
        args.save_path = os.path.join(args.out_path,f'checkpoint.torch')
    ...
    loaded =torch.load(args.save_path)
    print(args.save_path)
    model.load_state_dict(loaded)
    print(args.save_path)
    # print(torch.load(args.save_path))
    model.eval()
...
if __name__ == "__main__":
    ...
    if args.is_exception:
        ...
        for path in tqdm(test_game_paths):
            test_events+=(Game(path,args=args).output_events)
    ...
    if args.is_exception:
            ...
            if args.seen:
                out_file = open(f'seen_exception.csv','w')
            else:
                out_file = open(f'unseen_exception.csv','w')
            
    elif args.is_non_exception:
            if args.seen:
                out_file = open(f'seen_non_exception.csv','w')
            else:
                out_file = open(f'unseen_non_exception.csv','w')
        
    else:
            if args.seen:
                out_file = open(f'seen.csv','w')
            else:
                out_file = open(f'unseen.csv','w')
        
    print('ablation,loss,,belief,,,Act,,,dlg Dorothy,,,,,dlg Wizzard,,,,', file=out_file, flush=True)
    print('ablation,avg,stdev,own err,angle,gaol err,type,angle,acc,move,type,slot,total_rec,acc,move,type,slot,total_rec,acc', file=out_file, flush=True)
