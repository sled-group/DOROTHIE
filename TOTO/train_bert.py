...
def do_epoch(args, model, optimizer, criterion_cel, criterion_mse,criterion_bce, events, do_eval=False):
    ...
    return loss_lst, own_pos_err, own_angle_err, goal_pos_err, act_type_pairs, act_angle_err, dorothy_dlg_move_pairs, dorothy_type_rec,dorothy_slot_rec, wizzard_dlg_move_pairs, wizzard_type_rec,wizzard_slot_rec
...
def main(proc_idx, train_events, val_events, args):
    ...
    if proc_idx is None:
        ...
        out_file = open(f'{args.save_path.rsplit(".",1)[0]}.log','w')
        close_file = True
    ...
    for epoch in range(max_num_epochs):
        ...
        print("one_train")
        loss_lst, own_pos_err, own_angle_err, goal_pos_err, act_type_pairs, act_angle_err, dorothy_dlg_move_pairs, dorothy_type_rec,dorothy_slot_rec, wizzard_dlg_move_pairs, wizzard_type_rec,wizzard_slot_rec = output
        # for o in output:
        #     print(len(o))
    ...
    # output = do_epoch(args, model, optimizer, criterion_cel, criterion_mse, games*8, do_eval=True)
    # loss_lst, own_pos_err, own_angle_err, goal_pos_err, act_type_pairs, act_angle_err, dorothy_dlg_type_pairs, dorothy_slot_rec, wizzard_dlg_type_pairs, wizzard_slot_rec = output
    # # for o in output:
    # #     print(len(o))
    ...
    #     f'{np.mean(own_angle_err):8.3f}',
    #     f'{np.mean(goal_pos_err):8.3f}',
    #     f'{np.mean(act_type_pairs):8.3f}',
    #     f'{np.mean(act_angle_err):8.3f}',
    #     f'{np.mean(dorothy_dlg_type_pairs):8.3f}',
    #     f'{np.mean(dorothy_slot_rec):8.3f}',
    #     f'{np.mean(wizzard_dlg_type_pairs):8.3f}',
    #     f'{np.mean(wizzard_slot_rec):8.3f}',
    #     'Testing'
    #     )
...
if __name__ == "__main__":
    ...
    with open("train.json","r") as f:
        ...
        train_game_paths = json.load(f)
    # train_games = Pool(10).map(partial(Game, args=args),train_game_paths)\
    train_events=[]
    for path in tqdm(train_game_paths):
        train_events+=(Game(path,args=args).output_events)
    print('Loaded')

    print('Val Games',end=' ')
    with open("val.json","r") as f:

        val_game_paths = json.load(f)
    val_events=[]
    for path in tqdm(val_game_paths):
        val_events+=(Game(path,args=args).output_events)
    ...
    # train_multiplier = 100//len(train_games)
    # val_multiplier = 25//len(val_games)

    # train_multiplier = 5//len(train_games)
    # val_multiplier = 5//len(train_games)

    # manager = Manager()

    # train_games = manager.list(train_games)#*train_multiplier)
    # val_games = manager.list(val_games)#*val_multiplier)

    if args.ablation is not None and not args.parallel:
        main(args.ablation, train_events, val_events, args)
    else:
        # torch.multiprocessing.spawn(main, args=(train_games, val_games, args), nprocs=1, join=True)
        torch.multiprocessing.spawn(main, args=(train_events, val_events, args), nprocs=2, join=True)
        # torch.multiprocessing.spawn(main, args=(train_games, val_games, args), nprocs=4, join=True)
        # torch.multiprocessing.spawn(main, args=(train_games, val_games, args), nprocs=8, join=True)
