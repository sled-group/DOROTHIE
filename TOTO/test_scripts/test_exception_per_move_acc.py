...
def main(proc_idx, events, args):
    ...
    if proc_idx is None:
        ...
        if args.seen:
            out_file = open(f'seen_exception_specified_acc.csv','a')
        else:
            out_file = open(f'unseen_exception_specified_acc.csv','a')
...
if __name__ == "__main__":
    ...
    for path in tqdm(test_game_paths):
        test_events+=(Game(path,args=args,use_pretrained=True).exception_events)

    format_time = lambda x: f'{int(x)//3600:02d}:{int(x)%3600//60:02d}:{int(x%60):02d}'
    print('Loaded')
    print(format_time(time.time()-start))
    if args.seen:
        with open(f'seen_exception_specified_acc.csv','w') as f:
        ...
        with open(f'unseen_exception_specified_acc.csv','w') as f:
