...
def do_epoch(args, model, optimizer, criterion_cel, criterion_mse,criterion_bce, events, do_eval=False):
    ...
    act_angle_err = []
    act_per_move_accs=[]
    for i in range(len(PhysicalAction.__members__)):
...
def main(proc_idx, events, args):
    ...
    if proc_idx is None:
        ...
        if args.seen:
            out_file = open(f'seen_non_exception_specified_acc.csv','a')
        else:
            out_file = open(f'unseen_non_exception_specified_acc.csv','a')
