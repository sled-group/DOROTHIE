...
def do_epoch(args, model, criterion_cel, criterion_mse,criterion_bce, events, do_eval=False):
    times = []
    loss_lst = []
    own_pos_err = []
    own_angle_err = []
    ...
    act_angle_err = []
    act_per_move_accs=[]
    for i in range(len(PhysicalAction.__members__)-1):
        act_per_move_accs.append([])

    dorothy_dlg_move_pairs = []
    dorothy_type_rec = []
    dorothy_slot_rec = []
    dorothy_per_move_accs=[]
    for i in range(len(DialogueMove.__members__)):
        dorothy_per_move_accs.append([])
    dorothy_per_slot_f1=[]
    for i in range(len(DialogueSlot.__members__)-1):
        dorothy_per_slot_f1.append([])
    wizzard_dlg_move_pairs = []
    wizzard_type_rec = []
    wizzard_slot_rec = []
    wizzard_per_move_accs=[]
    for i in range(len(DialogueMove.__members__)):
        wizzard_per_move_accs.append([])
    wizzard_per_slot_f1=[]
    for i in range(len(DialogueSlot.__members__)-1):
        wizzard_per_slot_f1.append([])
    act_total_acc = []
    dorothy_total_acc=[]
    wizzard_total_acc =[]
    dorothy_slot_rec=[]
    ...
    for event in tqdm(events):
                ...
                slot_eval_fun = lambda x,y: [f1_score(a,b,zero_division=1) for a,b in zip(x,y)]
                ...
                if gt_physical_type !=None:
                    ...
                    # print(gt_physical_type)
                    if torch.argmax(physical_type,axis=-1)==gt_physical_type:
                        act_per_move_accs[gt_physical_type].append(1)
                    else:
                        act_per_move_accs[gt_physical_type].append(0)
                ...
                if gt_dorothy_dialogue_move !=None:
                    ...
                    # print(dorothy_dialogue_slot.shape,gt_dorothy_dialogue_slot.shape)
                    dorothy_type_rec += slot_eval_fun((dorothy_dialogue_type>0).cpu().data.numpy()*1,gt_dorothy_dialogue_type.cpu().data.numpy())
                    dorothy_per_slot_f1[0]+=slot_eval_fun((dorothy_dialogue_slot[:,0:10]>0).cpu().data.numpy()*1, gt_dorothy_dialogue_slot[:,0:10].cpu().data.numpy() )
                    dorothy_per_slot_f1[1]+=slot_eval_fun((dorothy_dialogue_slot[:,10:26]>0).cpu().data.numpy()*1, gt_dorothy_dialogue_slot[:,10:26].cpu().data.numpy() )
                    dorothy_per_slot_f1[2]+=slot_eval_fun((dorothy_dialogue_slot[:,26:32]>0).cpu().data.numpy()*1, gt_dorothy_dialogue_slot[:,26:32].cpu().data.numpy() )
                    dorothy_per_slot_f1[3]+=slot_eval_fun((dorothy_dialogue_slot[:,32:44]>0).cpu().data.numpy()*1, gt_dorothy_dialogue_slot[:,32:44].cpu().data.numpy() )
                    dorothy_per_slot_f1[4]+=slot_eval_fun((dorothy_dialogue_slot[:,44:]>0).cpu().data.numpy()*1, gt_dorothy_dialogue_slot[:,44:].cpu().data.numpy() )
                   ...
                   # print(torch.eq((dorothy_dialogue_type>0)*1,gt_dorothy_dialogue_type))
                    ...
                    # print(torch.eq((dorothy_dialogue_slot>0)*1,gt_dorothy_dialogue_slot))
                    if torch.argmax(dorothy_dialogue_move,axis=-1)==gt_dorothy_dialogue_move:
                        dorothy_per_move_accs[gt_dorothy_dialogue_move].append(1)
                    else:
                        dorothy_per_move_accs[gt_dorothy_dialogue_move].append(0)
                    # print(gt_dorothy_dialogue_type.shape)
                    # print(dorothy_dialogue_type.shape)
                    # print(len(dorothy_per_slot_accs),len(dorothy_per_slot_accs[0]),len(dorothy_per_slot_accs[1]))
                    # print(np.mean(dorothy_per_slot_accs[0]),np.mean(dorothy_per_slot_accs[1]))
                ...
                if gt_wizzard_dialogue_type !=None:
                    ...
                    wizzard_per_slot_f1[0]+=slot_eval_fun((wizzard_dialogue_slot[:,0:10]>0).cpu().data.numpy()*1, gt_wizzard_dialogue_slot[:,0:10].cpu().data.numpy() )
                    wizzard_per_slot_f1[1]+=slot_eval_fun((wizzard_dialogue_slot[:,10:26]>0).cpu().data.numpy()*1, gt_wizzard_dialogue_slot[:,10:26].cpu().data.numpy() )
                    wizzard_per_slot_f1[2]+=slot_eval_fun((wizzard_dialogue_slot[:,26:32]>0).cpu().data.numpy()*1, gt_wizzard_dialogue_slot[:,26:32].cpu().data.numpy() )
                    wizzard_per_slot_f1[3]+=slot_eval_fun((wizzard_dialogue_slot[:,32:44]>0).cpu().data.numpy()*1, gt_wizzard_dialogue_slot[:,32:44].cpu().data.numpy() )
                    wizzard_per_slot_f1[4]+=slot_eval_fun((wizzard_dialogue_slot[:,44:]>0).cpu().data.numpy()*1, gt_wizzard_dialogue_slot[:,44:].cpu().data.numpy() )
                    # print(len(wizzard_per_slot_f1))
                    # print(len(wizzard_per_slot_f1[0]))
                    # print(len(wizzard_per_slot_f1[1]))
                    
                    wizzard_total_slot_rec+=[torch.all(torch.eq((wizzard_dialogue_slot>0)*1,gt_wizzard_dialogue_slot)).cpu().numpy()]
                    
                    if torch.argmax(wizzard_dialogue_move,axis=-1)==gt_wizzard_dialogue_move:
                        wizzard_per_move_accs[gt_wizzard_dialogue_move].append(1)
                    else:
                        wizzard_per_move_accs[gt_wizzard_dialogue_move].append(0)
    ...
    return act_per_move_accs,dorothy_per_move_accs,wizzard_per_move_accs,dorothy_per_slot_f1,wizzard_per_slot_f1
def main(proc_idx, events, args):
    ...
    if proc_idx is None:
        ...
        if args.is_exception:
            if args.seen:
                out_file = open(f'seen_exception_acc.csv','a')
            else:
                out_file = open(f'unseen_exception_acc.csv','a')            
        elif args.is_non_exception:
            if args.seen:
                out_file = open(f'seen_non_exception_acc.csv','a')
            else:
                out_file = open(f'unseen_non_exception_acc.csv','a')   
        else:
            if args.seen:
                out_file = open(f'seen_acc.csv','a')
            else:
                out_file = open(f'unseen_acc.csv','a')
        close_file = True
    ...
    output = do_epoch(args, model,  criterion_cel, criterion_mse,criterion_bce, events, do_eval=True)
    
    act_per_move_accs,dorothy_per_move_accs,wizzard_per_move_accs,dorothy_per_slot_f1,wizzard_per_slot_f1=output
        # for o in output:
        #     print(len(o))
    # print(dorothy_per_slot_pred.shape)
    ...
    print(  f'ablation{proc_idx}_seed{args.seed},',
            ...
            f'{np.mean(act_per_move_accs[1]):.3f},',
            f'{np.mean(act_per_move_accs[2]):.3f},',
            f'{np.mean(act_per_move_accs[3]):.3f},',
            f'{np.mean(act_per_move_accs[4]):.3f},',
            f'{np.mean(act_per_move_accs[5]):.3f},',
            f'{np.mean(dorothy_per_move_accs[0]):.3f},',
            f'{np.mean(dorothy_per_move_accs[1]):.3f},',
            f'{np.mean(dorothy_per_move_accs[2]):.3f},',
            f'{np.mean(dorothy_per_move_accs[3]):.3f},',
            f'{np.mean(dorothy_per_move_accs[4]):.3f},',
            f'{np.mean(dorothy_per_move_accs[5]):.3f},',
            f'{np.mean(dorothy_per_move_accs[6]):.3f},',
            f'{np.mean(dorothy_per_move_accs[7]):.3f},',
            f'{np.mean(dorothy_per_move_accs[8]):.3f},',
            f'{np.mean(dorothy_per_move_accs[9]):.3f},',
            f'{np.mean(dorothy_per_move_accs[10]):.3f},',
            f'{np.mean(dorothy_per_move_accs[11]):.3f},',
            f'{np.mean(dorothy_per_move_accs[12]):.3f},',
            f'{np.mean(dorothy_per_move_accs[13]):.3f},',
            f'{np.mean(dorothy_per_move_accs[14]):.3f},',
            f'{np.mean(wizzard_per_move_accs[0]):.3f},',
            f'{np.mean(wizzard_per_move_accs[1]):.3f},',
            f'{np.mean(wizzard_per_move_accs[2]):.3f},',
            f'{np.mean(wizzard_per_move_accs[3]):.3f},',
            f'{np.mean(wizzard_per_move_accs[4]):.3f},',
            f'{np.mean(wizzard_per_move_accs[5]):.3f},',
            f'{np.mean(wizzard_per_move_accs[6]):.3f},',
            f'{np.mean(wizzard_per_move_accs[7]):.3f},',
            f'{np.mean(wizzard_per_move_accs[8]):.3f},',
            f'{np.mean(wizzard_per_move_accs[9]):.3f},',
            f'{np.mean(wizzard_per_move_accs[10]):.3f},',
            f'{np.mean(wizzard_per_move_accs[11]):.3f},',
            f'{np.mean(wizzard_per_move_accs[12]):.3f},',
            f'{np.mean(wizzard_per_move_accs[13]):.3f},',
            f'{np.mean(wizzard_per_move_accs[14]):.3f},',
            f'{np.mean(dorothy_per_slot_f1[0]):.3f},',
            f'{np.mean(dorothy_per_slot_f1[1]):.3f},',
            f'{np.mean(dorothy_per_slot_f1[2]):.3f},',
            f'{np.mean(dorothy_per_slot_f1[3]):.3f},',
            f'{np.mean(dorothy_per_slot_f1[4]):.3f},',
            f'{np.mean(wizzard_per_slot_f1[0]):.3f},',
            f'{np.mean(wizzard_per_slot_f1[1]):.3f},',
            f'{np.mean(wizzard_per_slot_f1[2]):.3f},',
            f'{np.mean(wizzard_per_slot_f1[3]):.3f},',
            f'{np.mean(wizzard_per_slot_f1[4]):.3f},',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_exception",default=False)
    parser.add_argument("--is_non_exception",default=False)
    ...
    if args.seen:
        with open("seen.json","r") as f:
            ...
            test_game_paths = json.load(f)
    else:
        with open("unseen.json","r") as f:

            test_game_paths = json.load(f)
    test_events=[]
    for path in tqdm(test_game_paths):
        test_events+=(Game(path,args=args,use_pretrained=True).output_events)
    ...
    if args.is_exception:
            if args.seen:
                out_file = open(f'seen_exception_acc.csv','w')
            else:
                out_file = open(f'unseen_exception_acc.csv','w')            
    elif args.is_non_exception:
            if args.seen:
                out_file = open(f'seen_non_exception_acc.csv','w')
            else:
                out_file = open(f'unseen_non_exception_acc.csv','w')   
    else:
            if args.seen:
                out_file = open(f'seen_acc.csv','w')
            else:
                out_file = open(f'unseen_acc.csv','w')
    
    print('ablation,Act,,,,,dlg Dorothy,,,,,,,,,,,,,,,dlg Wizzard,,,,,,,,,,,,,,,slot dorothy,,,,slot wizzard,,,,', file=out_file, flush=True)
    print(',LaneSwitch,UTurn,JTurn,Stop,Start,Irrelevant,Instruct,Explain,Align,Check,QueryYN,QueryW,Acknowledge,Confuse,Clarify,Confirm,ReplyY,ReplyN,ReplyU,ReplyW,Irrelevant,Instruct,Explain,Align,Check,QueryYN,QueryW,Acknowledge,Confuse,Clarify,Confirm,ReplyY,ReplyN,ReplyU,ReplyW,Action,Street,Landmark,Status,Object,Action,Street,Landmark,Status,Object,', file=out_file, flush=True)
