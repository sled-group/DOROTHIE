...
class CustomRecognizer(sr.Recognizer):
    ...
    def listen_till_signal(self, source, q, timeout=None):
        ...
        while True:
            ...
            # end of the phrase
            if pause_count > pause_buffer_count: pass
...
def run(cmd_queue, inbox, outbox, session_id):
    ...
    def recognize_loop(outbox, cmd_queue):
        r = CustomRecognizer()
        ...
        while 1:
            try:
                ...
                print('Listening...')
                with sr.Microphone() as source:
                    sr_audio = r.listen_till_signal(source, cmd_queue)

                res = r.recognize_google(sr_audio)
                outbox.put(res)
                print('You: %s' % res)

                with open('co_wizardlogs/log_%d/co_wizard_audio/utterance_%d.flac' % (session_id, time.time()), 'wb') as audiofile:
                    audiofile.write(sr_audio.get_flac_data())
            except Exception as e:
                print('Exception in recognize loop: ', str(e))

    recthread = threading.Thread(target=recognize_loop, args=(outbox, cmd_queue), daemon=True)
    recthread.start()

    while 1:
        msg = inbox.get()
        print('got utterance in wss: %s' % msg)
